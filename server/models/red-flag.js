...
const createRedFlag = async intervention => {
  ...
  const sql =
    'INSERT INTO red_flags(created_on, created_by, location, status, images, videos, comment) VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING *';
  const values = [
    createdOn,
    createdBy,
    location,
...
const findAll = async () => {
  ...
  const sql = 'SELECT * FROM red_flags';
  try {
...
const findById = async id => {
  ...
  const sql = 'SELECT * FROM red_flags WHERE id = $1';
  const values = [id];
  try {
